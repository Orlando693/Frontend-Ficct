...
export default function DecanatoLayout() {
  const role = localStorage.getItem("role") || "Decanato"
      ...
      <div className="lg:grid lg:grid-cols-[260px_minmax(0,1fr)]">
        ...
        >
          ...
          <nav className="mt-4 space-y-2 font-medium">
            <NavLink to="/decanato" className={({isActive}) => `${linkBase} ${isActive ? linkActive : ""}`} onClick={() => setIsSidebarOpen(false)}>
              <LayoutDashboard className="w-5 h-5" />
              Resumen
            </NavLink>
            <NavLink to="/decanato/reportes" className={({isActive}) => `${linkBase} ${isActive ? linkActive : ""}`} onClick={() => setIsSidebarOpen(false)}>
              <BarChart3 className="w-5 h-5" />
              Gestión de Reportes
            </NavLink>
            <NavLink to="/decanato/bitacora" className={({isActive}) => `${linkBase} ${isActive ? linkActive : ""}`} onClick={() => setIsSidebarOpen(false)}>
          ...
          <div className="mt-auto pt-6">
            <button onClick={() => logout()} className="w-full inline-flex items-center justify-center gap-2 px-3 py-2 rounded-xl bg-white/10 hover:bg-white/15 text-white">
