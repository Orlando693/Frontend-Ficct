...
export default function CareerFormModal({
  ...
  useEffect(() => {
    if (open && initial) {
      ...
      setNombre(""); setSigla(""); setEstado("ACTIVA"); setError(null)
    }
  }, [open, initial])
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault()
    setError(null)
    if (!nombre.trim() || !sigla.trim()) { setError("Nombre y sigla son obligatorios."); return }
    setSaving(true)
    try {
      ...
      <div className="w-full max-w-lg bg-white rounded-2xl shadow-xl p-6">
        ...
        <form onSubmit={submit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input className="w-full border rounded-lg px-3 py-2 uppercase" value={sigla} onChange={e=>setSigla(e.target.value.toUpperCase())} />
          </div>
          <div>
          ...
          <div className="sm:col-span-2 flex justify-end gap-2 pt-2">
            ...
            <button disabled={saving} className="px-4 py-2 rounded-lg bg-indigo-600 text-white">{saving ? "Guardando..." : "Guardar"}</button>
          </div>
        </form>
      </div>
    </div>
