...
export default function CarrerasPage() {
  ...
  function toneEstado(s: EstadoCarrera) { return s === "ACTIVA" ? "green" : "red" }

  async function crear(payload: Omit<Carrera, "id" | "materiasAsociadas" | "gruposAsociados">) {
    const c = await api.create(payload); setData(d => [c, ...d]); setMsg("Carrera creada.")
  }
  async function editar(payload: Partial<Carrera>) {
    const c = await api.update(payload.id!, payload); setData(d => d.map(x => x.id === c.id ? c : x)); setMsg("Carrera actualizada.")
  }
  async function setEstado(c: Carrera, estado: EstadoCarrera) {
    const up = await api.setEstado(c.id, estado); setData(d => d.map(x => x.id === up.id ? up : x))
    ...
    <div className="space-y-6 text-slate-800">
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h2 className="text-2xl font-semibold">Gestionar carreras</h2>
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-indigo-600 text-white font-medium hover:bg-indigo-700 shadow"
          >
      ...
      <div className="bg-white rounded-2xl shadow p-4 border border-slate-200">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <select
            className="px-4 py-2.5 rounded-lg border border-slate-300 text-slate-900 bg-white"
            value={fEstado}
          ...
          <table className="min-w-full text-sm">
            <thead className="text-left bg-slate-800 text-white">
              <tr>
                ...
                <th className="px-4 py-3 font-semibold">Asociaciones</th>
            ...
            <tbody className="text-slate-900">
              {view.map(c => (
                <tr key={c.id} className="border-b last:border-0 border-slate-200 hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3">{c.sigla}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        onClick={() => { setEditRow(c); setModalOpen(true) }}
                        className="px-3 py-1.5 rounded-lg bg-slate-900 text-white hover:bg-slate-800 inline-flex items-center gap-1.5 text-xs"
                        title="Editar"
                      >
                      ...
                      {c.estado === "ACTIVA" ? (
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-amber-600 text-white hover:bg-amber-700 inline-flex items-center gap-1.5 text-xs"
                          title="Inactivar"
                        >
                          <ToggleLeft className="w-3.5 h-3.5" /> Inactivar
                        </button>
                      ) : (
                        <button
                          onClick={() => setEstado(c, "ACTIVA")}
                          className="px-3 py-1.5 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 inline-flex items-center gap-1.5 text-xs"
                          title="Activar"
                        >
